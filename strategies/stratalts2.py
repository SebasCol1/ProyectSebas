...
def strat_EMA_RSI_CCI(df):
    ...
    if (float(EMAMedium(df.iloc[:-1, :], 23)) > float(EMAshort(df.iloc[:-1, :], 8))) and (float(EMAMedium(df, 23)) < float(EMAshort(df, 8))) and (
            float(RSI(df, 28)) > 50) and (float(CCI(df, 65)) > 80):
        return '🟢 Compra'

    elif (float(EMAMedium(df.iloc[:-1, :], 23)) < float(EMAshort(df.iloc[:-1, :], 8))) and (float(EMAMedium(df, 23)) > float(EMAshort(df, 8))) and (
            float(RSI(df, 28)) < 50) and (float(CCI(df, 65)) < 40):
        return '🔴 Venta'
...
def strat_EMA_RSI(df):
    ...
    print('strat_ema_rsi')
    print(df)
    #print(float(EMAMedium(df.iloc[:-1, :], 22)))
    #print(float(EMAshort(df.iloc[:-1, :], 5)))
    print(float(EMAMedium(df, 22)))
    print(float(EMAshort(df, 5)))

    if (float(EMAMedium(df.iloc[:-1, :], 22)) > float(EMAshort(df.iloc[:-1, :], 5))) and (float(EMAMedium(df, 22)) < float(EMAshort(df, 5))) and (
            float(RSI(df, 20)) > 50):
        return '🟢 Compra'

    elif (float(EMAMedium(df.iloc[:-1, :], 22)) < float(EMAshort(df.iloc[:-1, :], 5))) and (float(EMAMedium(df, 22)) > float(EMAshort(df, 5))) and (
            float(RSI(df, 20)) < 50):
        return '🔴 Venta'
...
def strat_MA_Ribbon(df):
    ma_5 = 7
    ma_21 = 24
    ema_5 = talib.EMA(df['Close'], timeperiod=ma_5)
    ema_21 = talib.EMA(df['Close'], timeperiod=ma_21)


    if (float(EMAMedium(df.iloc[:-1, :], 24)) > float(EMAshort(df.iloc[:-1, :], 7))) and (float(EMAMedium(df, 24)) < float(EMAshort(df, 7))) and (float(RSI(df, 20)) < 50):
        return '🟢 Compra'
    elif (float(EMAMedium(df.iloc[:-1, :], 24)) < float(EMAshort(df.iloc[:-1, :], 7))) and (float(EMAMedium(df, 24)) > float(EMAshort(df, 7))) and (float(RSI(df, 20)) > 50):
        return '🔴 Venta'
    else:
        return '❌'
